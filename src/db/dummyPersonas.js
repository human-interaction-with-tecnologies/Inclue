
export const dummyPersonas = [
  {
    id: 1,
    name: "Maria Nazaré",
    description: "Uma empreendedora de 64 anos, residente da ilha do Sabiá, que trabalha na extração de açaí com seus famíliares. Recentemente, ela recebeu um smartphone de presente de Natal de seu irmão, seu primeiro dispositivo com tal tecnologia. Ela está ansiosa para aprender a usar o Pix, para facilitar os pagamentos dos clientes na feira do Ver-o-Peso. Contudo, ela indicou que tem receios de utilizar o aplicativo, devido não compreender como se utilizar as funções de teclado, botões e outros recursos tradicionais do smartphone.",
    characteristics: ["64 anos", "Extrativista de acaí", "Usuária novata", "Dificuldades com recursos tradicionais de smartphones"],
    image: "/images/personas/persona-1.jpg"
  },
  {
    id: 2,
    name: "José Silva",
    description: "José tem 50 anos e exerce a profissão de cozinheiro na cidade de Itapiri. Natural da região nordeste do país, mudou-se para a região sudeste aos 25 anos, por convite de um amigo, a fim de abrir um restaurante de comidas típicas. Em 2020, o ambiente de trabalho de José foi afetado pela pandemia de Covid-19, levando-o a receber auxílios emergenciais para ajudar na renda familiar e no seu negócio. Apesar disso, ele mencionou que utilizava o smartphone para enviar áudios aos seus filhos e atender às ligações dos clientes de seu restaurante. Dessa forma, solicitou o suporte de seus filhos para aprender a utilizar aplicativos de auxílios financeiros e de pagamentos instantâneos.",
    characteristics: ["Empreendedor e Cozinheiro", "50 anos", "Pouco contato com Smartphones", "Utiliza sistema de pagamento com suporte de familiares"],
    image: "/images/personas/persona-2.jpg"
  },
  {
    id: 3,
    name: "Keila Souza",
    description: "Keila tem 52 anos e atua como catadora na região metropolitana de Belém. Ela estudou até a quinta série do ensino fundamental, aos 11 anos, devido à necessidade de ajudar sua mãe, que trabalhava na zona rural, a criar seus irmãos. Seus irmãos são gratos a ela pelo esforço e, recentemente, a presentearam com uma casa próxima a uma escola de ensino fundamental e médio. Este ano, Keila começou a estudar na escola, e os professores a motivaram a utilizar o Pix como forma de aumentar sua renda. Assim, ela tem aprendido a utilizar o Pix e, sempre que tem dúvidas, recorre às professoras Joana e Carol, de português e informática.",
    characteristics: ["Catadora", "52 anos", "Ensino Fundamental Incompleto","Dúvidas para compreender textos e ícones"],
    image: "/images/personas/persona-3.jpg"
  },
  {
    id: 4,
    name: "João Santana",
    description: "João, aos 38 anos e com dois filhos, trabalha na agricultura em Fontes Verdes. Há dois anos atrás, ele utilizava combustível para obter energia em suas tarefas diárias, devido à ausência de eletricidade na região. No ano passado, seu filho mais velho mudou-se para a zona urbana, a 247 km de distância. Para manter contato, João adquiriu um smartphone juntamente com uma antena de longo alcance para internet. Devido à distância, comodidade e por questões de segurança, ele abriu uma conta em um banco digital, utilizando-a para pagar contas e fornecer suporte financeiro ao filho. Inicialmente, João enfrentou dificuldades com o aplicativo bancário, mas a assistência do filho mais novo tem reduzido esses obstáculos.",
    characteristics: ["Agricultor","38 anos", "Pouco contato com serviços de internet", "Dificuldade no uso de aplicativos bancários"],
    image: "/images/personas/persona-4.jpg"
  },
  {
    id: 5,
    name: "Fernanda Duarte",
    description: "Fernanda, aos 43 anos, é mãe de quatro filhos e trabalha como diarista. Devido a pandemia de Covid-19, tornou-se beneficiária de um auxílio fornecido pelo poder público. Entretanto, nos primeiros meses, não recebeu o benefício por enfrentar algumas dificuldades no acesso ao aplicativo de cadastro social. Com o auxílio de um amigo de confiança, Fernanda superou essas barreiras, realizou seu cadastro e teve sua aprovação para receber o auxílio. No entanto, expressou ao amigo o desejo de receber algumas aulas sobre o aplicativo, pois enfrenta dificuldades para preencher formulários e compreender ícones e textos.",
    characteristics: ["Diarista", "43 anos", "beneficiária de auxílio social", "Dificuldades de interação com formulários, ícones e textos"],
    image: "/images/personas/persona-5.jpg"
  },
  {
    id: 6,
    name: "Karina Oliveira",
    description: "Karina, aos 43 anos, é sacoleira. No mês passado, durante as aulas da Escola de Jovens e Adultos, ela recebeu instruções sobre empreendedorismo e educação financeira. Desse modo, tem buscado alternativas para inovar em suas vendas diárias. Com a ascensão do Pix, ela abriu uma conta e está empenhada em aprender sobre o aplicativo. Contudo, Karina afirma que tem pouca experiência com smartphones, com aproximadamente um ano de contato. Logo, abriu uma conta bancária mas sente que o aplicativo não fornece dicas de uso e, assim, enfrenta dificuldades para inserir dados em campos de texto e compreender as ilustrações.",
    characteristics: ["Vendedora", "43 anos", "Estudante da EJA", "Pouca experiência com smartphones", "Dificuldades de compreensão com campos de texto e ilustrações"],
    image: "/images/personas/persona-6.jpg"
  }
];
